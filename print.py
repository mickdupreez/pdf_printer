...
def print_files_with_keywords(
    downloads_dir='~/Downloads',
    printer_name='HP_LaserJet_M402dw__031836_', # <-- This is where you set your printer name, replace spaces with underscores
    keywords=['WorkConfirmation', 'label_', '_shipper', 'PackingList'] # <-- Add or change keywords here
    ): 
    """
    ...
    - keywords (list[str]): A list of keywords to search for in the file names.
    Default is ['WorkConfirmation', 'label_', '_shipper', 'PackingList'].

    Returns:
    - None. If the specified directory does not exist or the program does not have permission to access it,
    returns an error message.If the specified printer is not available, returns an error message.
    If there was an error while printing a file, returns an error message.
    """
